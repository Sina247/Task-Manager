...
document.addEventListener("DOMContentLoaded", function () {
	...
	async function createTask(taskData) {
		const imageBase64 = taskData.imageFile ? await convertImageToBase64(taskData.imageFile) : null;
		const newTask = {
	...
	function renderTask(task, isUpdate = false) {
		if (isUpdate) {
			...
			if (existingTask) {
				const imageHtml = task.image ? `<img src="${task.image}" alt="Task image" class="task__image">` : "";
